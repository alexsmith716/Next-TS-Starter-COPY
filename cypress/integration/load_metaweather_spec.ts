context('Index page - MetaWeather Component - Dispatch loadMetaWeather() - Success', () => {
	beforeEach(() => {
	...
	it('cy.contains() - verify MetaWeather component Reload button', () => {
		cy.get('body > div').children().then(($div) => {
	...
	it('cy.click() - cy.wait() - click MetaWeather component Reload button and verify successful re-render', () => {
		cy.get('body > div').children().then(($div) => {
			cy.get($div[4]).children().first().find('button').click();
			cy.wait(3250);
			cy.get($div[4]).children().first().find('span').should('have.attr', 'data-cy');
		})
	});
});
